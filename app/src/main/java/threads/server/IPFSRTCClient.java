 ...
 * connections.
 */
public class IPFSRTCClient implements RTCClient, Session.Listener {
    ...
    static final Pattern IP_PATTERN = Pattern.compile("("
            ...
            + "(:(\\d+))?");
    private static final String TAG = "IPFSRTCClient";
    ...
    public IPFSRTCClient(PID user, SignalingEvents events) {
        this.events = events;
        this.user = user;
...
/*
    ...
    if (!matcher.matches()) {
      reportError("roomId must match IP_PATTERN for IPFSRTCClient.");
      return;
    }
